...
from .websockets import GzipClientProtocol
...
class managed_ws_conn(object):
    ...
    def __call__(self, fn):
        ...
        async def async_decorated_fn(*args, **kwargs):
            ...
            with KDRBusyContext():
                async with WSConnectionContext(GzipClientProtocol, self.endpoint) as active_ws:
                    try:
                        ...
                        write_to_log(f'The above exception has prevented the "{fn}" ',
                            'method from firing correctly.',
